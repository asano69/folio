...
- 現在のオープンソース（OSS）書籍ビューアの多くは、基本的な表示・整理機能に特化しており、研究や学習に不可欠なメモ、マーカー、タグ付けといった注釈機能の統合が不十分である。
- 技術的側面では、シングルページアプリケーション（SPA）として構築されたシステムが主流であり、各ページに対するURIの永続性や参照可能性への配慮が欠けている。このため、特定の箇所へのリンクが将来的に損なわれるリスクを内包しており、学術的な資料基盤としての信頼性に課題が残る。
