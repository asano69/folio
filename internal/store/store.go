...
// Book is the DB representation of a scanned CBZ.
type Book struct {
	ID     string
	Title  string
	Source string
...
// Page is the DB representation of a single image entry inside a CBZ.
type Page struct {
	ID       int
	BookID   string
	Number   int
...
// GetFirstPage returns the first page of a book, or nil if the book has no pages.
func (s *Store) GetFirstPage(bookID string) (*Page, error) {
	var p Page
	err := s.db.QueryRow(`
		SELECT id, book_id, number, filename
		FROM pages
